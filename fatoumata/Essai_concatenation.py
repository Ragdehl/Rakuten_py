...
navigation = st.sidebar.radio('',["Introduction", "DataViz", "Exploration Données", "Méthode", "Démonstration et résultats", "Jeu" , "Bilan"])
...
if navigation == "Exploration Données":
    os.chdir('C:\\Users\\barry\\OneDrive - CSTBGroup\\ds_images')
...
if navigation == "Démonstration et résultats":
    ...
    st.title('Prédiction du modèle concatenate')
    ...
    with st.form('a') :
        ...
        if st.form_submit_button('Image - Architecture du modèle') :
            st.image(image, 300)
    ...
    st.write("Classe " + y_pred[0] + " :" +classes[y_pred[0]])
...
#Create a column with the name of the picture
    ...
    def show_product(i = st.number_input("Entrer un identifiant de produit compris entre X et Y :", 0)):
    ...
    def classification_manuelle(i,type_pred, prediction):
        ...
        st.text('')
        st.text('Vraie classe: ' + y[i] + ' ; ' + classes[y[i]] \
            + '\n' + 'Votre prediction: ' + prediction + ' ; ' + classes[prediction] \
            + '\n' + 'Prédiction du modèle concatenate: ' + str(y_model[i]) + ';'  + classes[str(y_model[i])] )
    ...
    if navigation == "Jeu" :
        with st.form("S1"):
            ...
            if pred not in ['***Livres***', '***Jeux***', '***Jouets et Figurines***'] :
                ...
                if st.form_submit_button('valider') :
                    ...
                    y_organised = [int(a) for a in y_organised]
                    ...
                    cnf_matrix = conf_matx(y_true,y_pred) #montre les résultats en pourcentages!    
        ...
        #tick_marks = set(y_test)
                    ...
                    for z, j in itertools.product(range(cnf_matrix.shape[0]), range(cnf_matrix.shape[1])):
                        plt.text(j, z, cnf_matrix[z, j],
                                 horizontalalignment="center",
                                 color="white" if cnf_matrix[z, j] > ( cnf_matrix.max() / 2) else "black")
